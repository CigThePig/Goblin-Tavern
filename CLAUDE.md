...
>
> The **Simulation Expansion and Obligation-Closure plan** is the only unpaused work. It is a standalone implementation document — no earlier audit, report, or conversation is needed to execute it — and it runs as **ISSUE-170…183 (repo phases 207–220)**, one issue per plan phase 0–13. **ISSUE-170…173 (Phases 0–3) are done — next is ISSUE-174 (Phase 4 — service flow, customer choice, active regulars).** `OBL-01` is closed and so is the staff half of `OBL-02`.
>
...
- **Phases 207–210 — the expansion arc so far (ISSUE-170…173, done).** The frozen baseline and implementation ledger; the seven shared contracts under `src/sim/contracts/` (typed scheduled events, obligations, persistent ruleset, causality, meters, the actor interface, architecture checks); functional areas with a complete upgrade lifecycle (**OBL-01 closed**); and a persistent workforce with employment terms, shifts, stations, absence, cross-training, promotion, relationships, wage arrears and real resignation — founding staff included (**staff half of OBL-02 closed**).
