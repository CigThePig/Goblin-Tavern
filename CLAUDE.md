...
>
> The **Simulation Expansion and Obligation-Closure plan** is the only unpaused work. It is a standalone implementation document — no earlier audit, report, or conversation is needed to execute it — and it runs as **ISSUE-170…183 (repo phases 207–220)**, one issue per plan phase 0–13. **ISSUE-170…177 (Phases 0–7) are done — next is ISSUE-178 (Phase 8 — the autonomous social world).** `OBL-01`, `OBL-03` and `OBL-04` are closed, and so are the staff, loan and eviction halves of `OBL-02`; Phase 5 also absorbs and closes ISSUE-168.
>
...
- **Phases 207–214 — the expansion arc so far (ISSUE-170…177, done).** The frozen baseline and implementation ledger; the seven shared contracts under `src/sim/contracts/` (typed scheduled events, obligations, persistent ruleset, causality, meters, the actor interface, architecture checks); functional areas with a complete upgrade lifecycle (**OBL-01 closed**); a persistent workforce with employment terms, shifts, stations, absence, cross-training, promotion, relationships, wage arrears and real resignation — founding staff included (**staff half of OBL-02 closed**); a capacity-constrained six-wave service flow with patron choice, patience, tabs and active regulars; a survival economy where persistent service quality changes demand/spend/terms, attributable operating costs reconcile daily/weekly/monthly, policies need real enforcers, and insolvency can constrain/close/restructure/recover the tavern (**ISSUE-168 closed**); and transactional procurement where quoted orders reserve capacity and credit, real reliability governs dated deliveries, delivered goods create payable invoices, and the player can amend, cancel, split, substitute, dispute, pay, schedule and negotiate (**OBL-04 closed**); and external obligations with real lifecycles — lenders, loans with schedules, delinquency, collections, renegotiation and settlement; a tenancy that bills real rent periods and runs a notice ladder ending in an eviction hearing the tavern can still escape; and an inspector who actually visits, grades seven dimensions against live state, and leaves findings, fines and orders a follow-up re-reads (**OBL-03 closed**, plus the loan and eviction halves of **OBL-02**).
