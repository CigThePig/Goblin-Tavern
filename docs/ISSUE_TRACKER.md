...
### ISSUE-155 — Causal establishing line
- **Grade:** thin · **Status:** done · **Phase:** 188 · **Record:** `docs/plans/phase-188-causal-establishing-line.md`
- **Evidence:** `seed.causes` (`issueSeedTypes.ts`/`TavernState.ts:424`) is populated by `generateCustomerComplaint` (`issueSeedGenerators.ts` ~1338) with classified, human-readable triggers, but `grep seed.causes src/cards/` was empty: the `customer_complaint` establishing pool composed only from bands/pressures/memory tags (`establishingLine.ts`), and the seed's `recentContext` was hardcoded to `'main room dirty all week'` / `problemNoun` to `'cold welcome'` (`issueSeedGenerators.ts` ~2049/2044) regardless of the real cause. Causes were computed as truth and discarded before reaching the card — Architectural Rule 6 half-applied.
...
- **Grade:** broken · **Status:** done · **Phase:** 197 · **Record:** `docs/plans/phase-197-cause-coverage-instrument-repair.md`
- **Evidence:** `buildCauseReport` read `ctx.getDiffs()` during `generateReports`, but `finalize('day')` runs after the phase loop in both engine entry paths — the unexplained-changes check had returned "(none)" on every simulated day. Behind it: core-slice `modify*` auto-causes used dot targets (`stock.<id>.<field>`) while both `targetForChange` (causeReport) and `pathToCauseTarget` (causeLookup) expect colon (`stock:<id>`) — 52% of day-1 causes unmatchable by audit AND UI drilldown; and `draft.target ?? defaults.target` precedence let caller meta clobber per-field targets (localArcs emitted `arc:<id>` instead of `local_event:<id>.<field>`). Empirics: 168-day runs showed 0.8% (6/793) player-facing unexplained under a convention-tolerant matcher, all from the clobbering instance; instruments at `scripts/resurrect-cause-audit.ts`, `scripts/true-gaps-audit.ts`.
- **Scope:** `ctx.getDiffSoFar(boundary)` backed by `ChangeTracker.diffAgainst`; causeReport switched to it (Cluster 1). Core emitters unified on the colon convention; shared `canonicalCauseTarget` (`src/sim/modules/causes/causeTargets.ts`) consumed by causeReport + causeLookup; transitional dot-tolerance in `targetMatches` (Cluster 2). `emitDiffPathCausesForRecord` sanitizes meta (`target`/`targetType`/`amount`) for per-field drafts; aggregate fallback unchanged (Cluster 3). Cluster 4: planted-gap engine test (both entry paths) + happy-path + clobber regression in `tests/sim/phase197.causeCoverage.test.ts`; `audit:cause-coverage` npm script wired to `scripts/true-gaps-audit.ts`, which now imports the shared canonicalizer instead of duplicating the map.
- **Depends on:** none.
- **Test approach:** a deliberately uncaused significant mutation yields `unexplainedCount > 0` in the live report (simulateDay AND segmented A/B/C); a `modifyStock` change yields `unexplainedCount === 0` and the colon auto-cause sits under the drilldown's shared `pathToCauseTarget` id; a clobber-baited `modifyLocalEvent` still emits `local_event:<id>.intensity` with the real delta. Both committed diagnostics report zero player-facing gaps over 3 seeds × 56 days. `npm test` + `npm run typecheck` green.

### ISSUE-165 — UX polish pass: ID leaks, theme unlock, CTA hierarchy, service feedback
- **Grade:** solid · **Status:** done · **Phase:** 198 · **Record:** this entry (external UX-redesign request; audit + fixes in one pass).
- **Evidence:** Player-visible regressions found by driving the real app: daily report printed raw sim ids ("Lifted by local_goblins.", "Dragged by chair_damage.", literal "Incident: event" twice — incidents carry `id`, never the `kind`/`type` the fallback looked for); ActionPicker showed engine rejection strings ("adjust_prices requires a numeric amount delta"); TopBar's day-type badge clipped at phone width ("Supplie…"); the parchment palette in `global.css` was unreachable (index.html pins `data-theme="dark"`, no theme preference existed) and chrome used raw palette tokens (`--ink`/`--ink-deep`) that don't flip; the service beat showed nothing while the topbar coin jumped; fonts loaded from Google's CDN (theme degrades to Georgia offline); StartScreen footer leaked dev jargon.
- **Scope:** `dailyReportProjection.ts` — incident lines humanize `kind ?? type ?? id` via `humanizeId`, skip label-less incidents, dedupe repeats with ×N; driver names resolve through `state.customerGroups[id].label` then `humanizeId` (new `incidentLabel`/`driverDisplayName` helpers). Same incident fallback in `surface/dayNarrativeProjection.ts`. Web boundary `humanizeActionReason` in `actionBuilder.ts`, applied in ActionPicker (disabled reasons + quote warnings, deduped via `visibleWarnings`), QuickActions, ProjectsPanel. TopBar: pressure chip is the only shrinking element (badge/day-chip `flex-shrink: 0`), label keeps `min-width: 6ch`, ≤480px drops the trend glyph. Theme preference (`theme: 'dark' | 'light' | 'auto'`, default `'dark'`) in preferences/prefsStore/App effect/SettingsSection segmented control; chrome and insets re-tokenized (`--surface`/`--bg`/text-tint tracks/neutral-black card insets) so parchment mode is fully readable. `gameStore.serviceOutcome` (ephemeral, from the service module's state slice after Segment B) renders a patrons/net-coin/incidents strip on the service beat. Filled primary CTAs (DayScreen/StartScreen/ActionPicker confirm; Quick Day stays outline). Fonts self-hosted via `@fontsource/{cinzel,eb-garamond,ibm-plex-mono}` imports in `main.ts` (CDN link removed). Card narrative previews: body italic + sentence-cased at render (mono stays on mechanical chips). Morning glance row gains micro-labels (Coin/Staff/Stock). BottomNav clusters tabs into the content column ≥720px. StartScreen footer re-flavoured.
