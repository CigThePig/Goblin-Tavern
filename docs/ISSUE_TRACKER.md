...
| ISSUE-134 | Faithful Surface Phase 4 — Flavor That Doesn't Lie (+ restored the Phase-1 legibility todo; the Phase-1/2/3 plans folded the leftover `it.todo` into this phase). Fixes the arc's defect class #4 (178 flavor-vs-sim contradictions) and drives the cross-template legibility gate to zero so the deferred `it.todo('every migrated situation passes Q1 + Q2')` is restored to a live assertion. **(1) New `simCoherence` flavor detector `unbacked_state_claim`** (`src/cards/compose/gates/simCoherence.ts`) — the fourth structural flavor detector, mirroring `history_claim` / `role_claim` (text scanner + condition-presence check). Revises the load-bearing "flavor makes no checkable claims" assumption: a flavor line that asserts how an actor *feels* makes a claim the sim must back. Two small, frozen, inspectable lexicons (`DEFAULT_DISTRESS_PATTERNS` ~17 phrases: `heavy hush`, `like ash`, `wrung out`, `worn through`, `pulled tight`, `strain`, `holding the line`, `shadows under the eyes`, `knuckles whiten`, `worn knot`, `tired hand`, …; `DEFAULT_CALM_PATTERNS` ~4: `rested`, `at ease`, `loose-shouldered`, `well-slept`) scoped to first-person / close-third WELLBEING — NOT narrator situation lines ("tension is rising"). A flavor snippet matching either lexicon must carry ≥1 `STATE_LOOKUP_KINDS` condition or it fails. Scoped to body flavor slots via `isConsequenceSlot` — the synthetic `choice_label::*` / `effect_preview::*` slots are exempt (a preview "Staff would carry the strain" describes a FUTURE mechanical consequence backed 1-to-1 by the effect, not current state). **(2) Flavor audit.** The detector flagged exactly 5 genuinely-ungated voice/tic-rung distress lines (the other ~636 flavor snippets were already state-gated by Phases 150/151/153, or make no situational claim): 3 in `staffAside/asideLine.ts` (`aside_florid` "Sleep tasted like ash", `aside_florid_open` "I've been carrying a heavy hush since dawn" — the arc's named offender, `aside_tic_understates` "A bit of strain, perhaps") + 2 in `staffBurnout/reactionLine.ts` (`rxn_florid`, `rxn_formal_restrained` "the strain bears mention"). These are personality rungs (the card fires regardless of distress; the dedicated `*_state_*` rungs from Phase 150 already carry the real distress) — reworded to remove the distress claim, keeping voice character ("Morning came dressed in grey…", "I shall serve, as duty plainly holds", etc.). **(3) Multi-fact join valence drop** (`src/cards/compose/assemble.ts` `pickSecondaryForMulti`) — a `'multi'` establishing slot no longer staples a secondary whose valence is the strict opposite of the primary. `readValence(read)` reuses Phase-164 `resolveMeterValence`: a SIGNAL band's good/bad pole is calm/distress; mid is neutral; PRESSURES are neutral (a rising trend orthogonally co-occurs with any current mood — "the wagons run steady, but the market's turning" is coherent). `valenceProfile` is computed over EVERY read a snippet covers (a mixed snippet like "steady steps, but the long week shows" holds both poles and is never a strict opposite), and `strictlyOpposed` drops only a pure-calm-vs-pure-distress SIGNAL pairing ("their patience snapped — they've never been steadier"). Conservative by construction so it never breaks the deliberately-orthogonal combos the staffAside / supplierReliability pools author. **(4) Legibility todo restored.** Drove the cross-template `checkLegibility` report from 202 → 0 violations: (a) **preview_cost_unsurfaced (173 → 0)** — templates pass `maxPreview: 2`, dropping a coin-cost effect at index ≥2 so the spend never surfaced. New shared `selectPreviewEffects(source, previewMax)` in `src/cards/compose/previewSelect.ts` (imported by BOTH `composeChoicesFromSeed` and the legibility gate so they never drift) pulls a negative-coin effect into the cap; the gate derives its backing effects via `selectPreviewEffects(source, lineCount)` to stay index-aligned with the rendered lines. (b) **preview_magnitude_missing (29 → 0)** — split two ways: the legibility magnitude rule now exempts `future_hook` / `cause` effect kinds (narrative recurrence hooks / attribution writes whose classified band is not player-facing magnitude, matching the previewVariety design that treats such effects as band-less); and a `classifyTargetKind` gap was fixed — `world.regulars.<id>.loyalty` (the state_change form of a named regular's meter) fell to `'other'` so no shared banded snippet matched, dropping the magnitude token; added `world.regulars.` / `regulars.` → `customer` so the Phase-158 customer banded snippets land. `it.todo` → live `it('every migrated situation passes Q1 + Q2')` asserting `report.pass` + zero per-reason tallies. **Tests:** 3 new simCoherence fixtures (distress-token-no-condition fails / +signalEquals passes / no-claim gesture stays silent), new `tests/cards/compose/phase166.multiFactValence.test.ts` (3 — strict-opposite drop, same-sign keep, determinism on the `regular_customer` table), 2 new `classifyTargetKind` cases (`world.regulars.*` → customer), the restored legibility assertion. **No new gates** (extends `simCoherence` + `legibility`), **no new condition primitives**, **no `EffectDirection` redefinition**, **no magnitude-lexicon change**, **no mechanical sim changes** (only the additive `classifyTargetKind` rule + valence-aware preview metadata). Full suite green + typecheck clean. Depends on ISSUE-131 (real-seed samplers) + ISSUE-132 (valence) + ISSUE-133 (label/preview distinctness). | broken | done | 166 |
| ISSUE-135 | Faithful Surface Phase 5 — Close the Loop. Wires the arc's audit into the standing bar so the structural blind spot that hid all four defect classes can't reopen. **New standing gate `checkFaithfulness`** (`src/cards/compose/gates/faithfulness.ts`, exported through `gates/index.ts`, NOT in `runAllGates`) — a pure cross-sim integration gate analogous to `crossSituation` / `legibility`: it consumes pre-rendered `FaithfulnessSample { seed, state, cardView }` and asserts exactly four rules (frozen `FAITHFULNESS_REASONS`), one per arc defect class. **Harness** `tests/cards/compose/gates/faithfulnessHarness.ts` drives `runCardlessSim` across four policy bots (`auto_clean_focused` / `auto_profit_focused` / `auto_staff_friendly` / `auto_ignore_repairs`, each resolving seeds via `chooseResponse` so meters move through bands) for ~84 days each, renders every non-fallback emitted seed via `pickCard`, and collects ~3,017 production-shape samples — memoised once at import, one bot-run held at a time so the heavy `CardlessRunResult` is GC-eligible before the next (the phase20 OOM pattern avoided). Routed into the vitest forks/isolated slow shard. **The four rules:** (a) `direction_mismatch` — a rendered preview line for a `METER_VALENCE`-resolving (lower-is-better) meter must not carry opposite-direction medium/large magnitude vocabulary (re-checking `effect.direction` is tautological post-Phase-2; the render-level signature is opposite-vocab at the bands where `MAGNITUDE_LEXICON` is directional); reuses `lineCarriesMagnitude`. (b) `label_collision` — two `(slotId,verb,targetId)`-distinct choices canonicalise to the same label (cross-sim analogue of legibility's check). (c) `duplicate_preview_line` — two distinct choices in one card render the same preview line. (d) `distress_state_mismatch` — a pure-distress flavor body line fires on a calm actor (or pure-calm on distressed), computed from STATE via `querySignal`/`pressureIsRising` (`actorDistressReading`, staff stress/fatigue high + `staff_burnout` rising / regular irritation; mid-band ⇒ skip); mixed lines (carrying both poles) are excluded, matching Phase-4's "preserve mixed snippets". **The switch to real production renders surfaced that defects #2/#3/#4 were NOT fully closed** (2,953 duplicate previews, 27 label collisions, 73 distress false-positives) — exactly the arc's thesis (per-template gates ran against shapes that didn't exhibit the cross-sim collisions). Per user direction (full fixes, not bandaids), all four driven to zero: **(1) within-card distinctness mechanism** (`src/cards/compose/assemble.ts` + `cardHelpers.ts`) — `pickSnippet`/`pickSnippetTrace` gain an optional `avoid: ReadonlySet<string>` (canonical forms of lines already emitted on the card); `composeChoicesFromSeed` threads a per-card `usedLabels` + `usedPreviews` set so a later choice prefers an unused TOP-specificity candidate over FNV-pigeonholing onto a used line (staying within the top tier preserves the magnitude token a banded effect needs — legibility-safe by construction; empty `avoid` ⇒ byte-identical pre-Phase-167). Within ONE choice, repeated same-cell effects reuse a single line (a per-choice `cellLine` map) so a choice consumes only one candidate per cell, leaving the budget for cross-choice distinctness. **(2) shared-base candidate authoring** — 10 new magnitude-bearing sibling snippets in `_shared/effectPreviewBase.ts` for the thin cells the audit flagged (area pos-small long_term ×3, culture pos-small ×2 + pos-medium ×1, customer pos-medium ×1, faction neg-large ×1, staff pos-large ×1) so the distinctness pass has enough top-tier candidates per cell. **(3) seasonal_arc label fix** — `choiceLabel.ts` gains a `responseShape: reputation_play` variant so `let_it_brawl` and `ignore_warning` (both verb `ignore`) read distinctly. **(4) distress lexicon expansion** — `simCoherence.__internal.DEFAULT_DISTRESS_PATTERNS` extended with the staff burnout/aside idioms (`wound tight`, `tight shoulders`, `set jaw`, `tight as a wire`, `bone-tired`, `wearing them through`, `crowding into`, `still climbing`) so coherent concessive distress lines ("Rested enough, and still wound tight at the rota") register as mixed (carrying both poles) rather than false-flagging as pure-calm on a high-stress actor — strengthens the static `unbacked_state_claim` detector too. **Tests:** new `tests/cards/compose/gates/faithfulness.test.ts` — 5 live tests (all four rules pass on ~3,017 renders; canary `samplesEvaluated ≥ 2000`; non-vacuous `directionChecksRun`/`distressChecksRun > 0`; determinism; frozen tuple) + 4 failure fixtures (one per reason). **No new condition primitives**, **no `EffectDirection` redefinition**, **no `runAllGates` shape change**, **no sim-side mechanical changes** (slotId/verb/targetId/effect amounts untouched — distinctness is presentation-only). Full suite green at 3,161/3,161 across 228 files + typecheck clean; legibility's 10 live tests stay green throughout. Depends on ISSUE-131…134 (the full arc). | thin | done | 167 |
| ISSUE-136 | Complete Surface Phase 1 — The Gate-Wiring Contract. Opens the [Complete Surface arc](plans/complete-surface-arc.md). Audit (2026-05-29) found the quality gates are wired by convention, not contract: `previewVariety` / `choiceDistinctness` / `reportLegibility` are opt-in and configured for **zero / one-synthetic / report-only** real templates, so the 20 card templates' per-template `runAllGates` walks run only the seven always-on framework gates; preview/choice quality is checked only by the cross-sim `legibility` / `faithfulness` harnesses, which are **hand-maintained registries** with no completeness check (a new template forgotten from a list loses protection silently). CLAUDE.md's Phase-161 claim that per-template `runAllGates` runs all ten gates is inaccurate. Scope: (a) a harness-completeness test deriving the required set from `REQUIRED_CARDS` and failing if any template is absent from a cross-sim harness; (b) wire `previewVariety` + `choiceDistinctness` into every applicable per-template `runAllGates` walk with its real sampler; (c) triage/fix in the pool (never loosen the gate) any defect the newly-run gates surface; (d) correct the CLAUDE.md claim. Cross-sim harnesses stay siblings. Depends on ISSUE-135 (Faithful arc complete). **Done (phase 168):** `tests/cards/compose/gates/fullGateHarness.ts` (`FULL_GATE_SITUATIONS` over the 20 migrated templates; generic `previewVarietySamplerFor` / `choiceDistinctnessSamplerFor` deriving the render from each seed's own `responseSlots` + `consequenceProfiles`, applying the production `applyLegibleChoiceCap` + uniform `maxPreview: 2`); `fullGateWiring.test.ts` runs the full nine-gate `runAllGates` walk per template (asserts `previewVariety`/`choiceDistinctness` `skipped === false` + pass); `harnessCompleteness.test.ts` derives `REQUIRED_IDS` from `REQUIRED_CARDS` and fails on any gap in `FULL_GATE_SITUATIONS` / `LEGIBILITY_SITUATIONS` / `cardRegistry` (faithfulness walk) / `CROSS_SITUATION_ACTOR_KINDS` (for the structurally-derived 11 actor-voiced templates) + no stale entries + records the `reportLegibility` card exclusion as data (`CARD_TEMPLATE_EXCLUDED_GATES`). No pool changes (the one gate firing was a `maxPreview` sampler artifact, not a real defect). CLAUDE.md Phase-161 claim corrected. 26 new tests. | broken | done | 168 |
| ISSUE-137 | Complete Surface Phase 2 — drinkOrder Parity. The one un-migrated flagship card. `drinkOrderCard` (`src/cards/templates/drinkOrder.ts:153-154`) splices raw `seed.textIngredients.recentContext[0]` into its body — an un-composed, un-gated, un-state-checked fragment, the exact class Voiced/Faithful killed elsewhere. It has no sim-backed establishing slot, no `saliencePolicy`, no salience-table read; the companion `sim_backed_hook` test (`tests/cards/compose/phase127.simBackedHookSignal.test.ts:57`) is still `it.skip`. Scope: add a `saliencePolicy:'multi'` `establishing_line` slot + matrix pool keyed on the `regular_customer` salient meters; delete the raw splice so the body is fully composed; re-enable the skipped test by seeding starter regulars and assert the signal resolves at render; run the full nine-gate walk + cross-sim harnesses. Mechanical choice fields untouched. Depends on ISSUE-136 (full gate set must run on drinkOrder as it lands). **Done (phase 169):** new `establishing_line` slot on `drinkOrderTemplate` (`role: utterance`, `wordBudget: 14`, `claimMode: sim_backed`, `saliencePolicy: multi`, `multiFactJoin: ' — '`) with a matrix pool at `src/cards/compose/pools/drinkOrder/establishingLine.ts` (16 snippets: fallback + single-condition rungs + the four reachable irritation × loyalty corners + two orthogonal signal×pressure/memory top rungs). drinkOrder is the MILD `relationship_test` branch (irritation ≤ 60), so the pool authors only the reachable face of the shared `regular_customer` cube — irritation ∈ {low, mid} × loyalty ∈ {low, mid, high}; the high-irritation row stays regularComplaint's, mid × mid stays on the fallback (full-cube fill deferred to Phase 6 / ISSUE-141). Raw `seed.textIngredients.recentContext[0]` splice at `drinkOrder.ts:153-154` deleted; body is now fully composed `[establishing_line, order_line, manner_note?]`. The dead `phase127.simBackedHookSignal.test.ts` skip guard (starter regulars are always seeded) removed; three new render-level assertions prove the salient reads resolve and the establishing line renders from the covering combo (not the bare fallback) for the starter regular's irritation=low × loyalty=high standing. `establishing_line` diversity slot added to drinkOrder's `runAllGates` block (`minDistinct: 1`, matching staffAside / regularComplaint). drinkOrder already present in every cross-sim harness (legibility / crossSituation / fullGate). Three body-index render tests in `templates.drinkOrder.test.ts` shifted (order_line is body[1] now). Spec `specs/cards/drink_order.spec.yaml` updated (establishing_line slot entry + phase169 design record). Full suite + typecheck green. | broken | done | 169 |
| ISSUE-138 | Complete Surface Phase 3 — Salience Completeness & the Reachability Allowlist. Arms Movement II. `policy_backlash` (`issueSeedTypes.ts:70`, generated at `expandedSeedGenerators.ts:4492`) has no `SALIENCE_TABLES` entry; it routes to fallback today but is a latent trap. Scope: add the missing entry + a derived test that fails if any family with an active generator has no salience table; add a pure, enumerable matrix-cell read over `SALIENCE_TABLES` + `BAND_THRESHOLDS`; add an `unreachableCells` allowlist (`Record<family, Cell[]>` with per-cell reasons) seeded with the audit's known picker-blocked cells. Ships reads + scaffold only — no content, no gate. Additive, deterministic. Depends on ISSUE-136. **Done (phase 170):** `policy_backlash` salience entry added to `src/cards/compose/salience.ts` (narrator-voiced — policy `primaryActor` carries no banded signal, so reads are `severity ≥ 70` crisis floor + the two embedded pressures `policy_backlash` / `faction_anger` + memory tags `warning` / `grudge` / `reversal` / `policy` mirroring the response-profile emissions + `repeat policy ≥ 3`; no new `SalienceRead` kinds). New `src/cards/compose/matrixCells.ts`: `MAX_MATRIX_METERS = 3`, `matrixMetersForFamily` (the salience table's `signal` reads in order, capped at 3, guarded against unbanded signals via `BAND_THRESHOLDS` membership), `matrixCellKey`, `enumerateMatrixCells` (cartesian product of `ALL_BAND_IDS` over the meters — `3^n` cells; narrator-voiced families → 0 cells; `inspection` caps its 4 signal reads to a 27-cell cube not 81). New `src/cards/compose/unreachableCells.ts`: `UnreachableCell` type, `UNREACHABLE_CELLS` allowlist **seeded empty** (the arc's "don't guess" rule dominates — the audit named gaps only in aggregate, and the doc's `rowdiness=low` example is selectable so not actually unreachable; Movement II populates per-cluster after picker instrumentation), `isCellAllowlisted`, and `validateUnreachableCells` (the real-cell invariant: every entry's `cellKey` must be an enumerable cell of its family — catches typo'd additions). All exported through `src/cards/compose/index.ts`. New `tests/cards/compose/phase170.salienceCompleteness.test.ts` (14 tests): derived completeness over `ALL_SEED_GENERATORS` (all 20 families covered), `policy_backlash` render resolution, matrix-cell counts/keys/determinism/cap, empty+validated allowlist. No content authored, no gate, conditions DSL untouched. Full suite + typecheck green. | thin | done | 170 |
| ISSUE-139 | Complete Surface Phase 4 — Suppliers, Stock & Debt matrix fill. Movement II content. Supplier 9-cell already strong (Legible Phase 4); fill or allowlist every stock-band and debt/rent cell so the Phase-3 cell-coverage read reports 100% (authored-and-reachable or allowlisted-unreachable, zero unexplained holes). Reaction/sensory pools vary with state. Full nine-gate walk + cross-sim harnesses green. Depends on ISSUE-138 (cell read + allowlist) + ISSUE-104/117 (the supplier/stock/debt templates). **Done (phase 171):** first application of the Phase-3 cell-coverage read to content. Under the read, the cluster's only band-enumerable matrix is `supplier_relationship`'s 9-cell `reliability × relationship` cube; `stock_shortage` and `debt_rent` are narrator-voiced (no banded subject signal — stock subject is a stock item, landlord is a `systemRef`) and enumerate to **0 cells by design**, their establishing coverage being over binary salient facts (severity floor / rising pressures / memories / `rent_due_soon` window / repeat) already authored exhaustively in Phase 149. The supplier 9-cell is **already covered to the cell** by the Phase-149 pool: four hand-authored combo corners, four single-meter edge snippets (each fires on its one non-mid meter regardless of the other), and the mid × mid centre on the documented fallback (matching the legibility gate's extremity-2 "meaningful read" filter — a mid-band signal read is the intentional default). Picker instrumentation (`generateSupplierRelationship`, `expandedSeedGenerators.ts:1127`) confirms all nine cells reachable — the rank score biases toward low reliability via `reliabilityDeficit` but does not floor it (blame/memory can lift a high-reliability supplier to the pick) and `relationship` is unscored — so `UNREACHABLE_CELLS` stays **empty** for the cluster (the arc's rule: allowlist only proven-unreachable cells). New `tests/cards/compose/phase171.matrixCoverage.test.ts` (16 tests) *derives* the supplier cells from `enumerateMatrixCells` (not a hard-coded 9) and asserts each resolves to a covering snippet (finite `scoreCandidateSalience` index) or the centre fallback; asserts stock/debt enumerate to 0 cells and still open on a salient binary fact; asserts `validateUnreachableCells()` clean with no cluster entries. No new snippets, no picker widening, no allowlist entries, no coverage gate (that is Phase 12 / ISSUE-147 — this ships the per-cluster proof it will generalise). Cluster's three templates already in `FULL_GATE_SITUATIONS` / `LEGIBILITY_SITUATIONS` / the faithfulness walk; nine-gate walk + harnesses + full suite + typecheck green. | thin | done | 171 |
| ISSUE-140 | Complete Surface Phase 5 — Staff & Personnel matrix fill. Fill the full `staff.stress × staff.fatigue` 9-cell grid on `staffAside` + `staffBurnout` (currently four corners + a few rungs; mid-band cells beyond the corners are the gap), or allowlist the unreachable ones. Reaction/sensory vary with state. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-118 (staff templates). **Resolved phase 172:** the audited gap was a single cell — the establishing pools authored `est_high_fatigue` but no `est_low_fatigue`, so `(stress=mid, fatigue=low)` had no covering snippet and fell to the bare fallback; added `est_low_fatigue` to both staff pools so all eight meaningful cells of the 9-cell stress×fatigue matrix resolve to a covering line, the all-mid centre stays on the fallback. Both pickers reach every cell so `unreachableCells` stays empty. Proven cell-by-cell in `tests/cards/compose/phase172.matrixCoverage.test.ts` (26 tests). **Deeper fix:** the calm `est_low_fatigue` primary exposed a pre-existing Phase-166 multi-fact-join hole — `readValence` (`assemble.ts`) treated all pressures as neutral, so a calm primary was stapled to a rising-`staff_burnout` distress secondary (a within-line self-contradiction no gate caught; the `est_low_stress` calm primary already triggered it). Added a narrow `SUBJECT_DISTRESS_PRESSURES` set (`staff_burnout`) so the join refuses that staple, matching the faithfulness gate's own `actorDistressReading`; 2 new `phase166.multiFactValence` cases pin it. Full suite 3249/3249, typecheck clean. | thin | done | 172 |
| ISSUE-141 | Complete Surface Phase 6 — Regulars & Complaints matrix fill. Fill `regular.irritation × regular.loyalty` and `customer_group.satisfaction × customer_group.loyalty`; verify the seed-firing-threshold-blocked cells (the complaint seed fires only above an irritation/dissatisfaction floor) and record them in `unreachableCells` rather than authoring dead rungs. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-119 (complaint templates). | thin | open | 173 |
| ISSUE-142 | Complete Surface Phase 7 — Factions & Culture matrix fill. Fill the `faction.relationship × faction.influence` 9-cell and the **27-cell `culture.tension × culture.comfort × culture.familiarity` cube** beyond the readable diagonal (the mid faces are the gap), or allowlist unreachable cells. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-120 (faction/culture templates). | thin | open | 174 |
| ISSUE-143 | Complete Surface Phase 8 — Premises & Atmosphere matrix fill. `maintenance` + `areaAtmosphere` author only the picker-dominant ~8 of 27 `area.condition × cleanliness × damage` cube cells. Instrument the area pickers; for each absent cell either widen the picker (additive sim change) so it is reachable and author it, or record it in `unreachableCells` with a verified reason. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-106/121 (premises templates). | thin | open | 175 |
| ISSUE-144 | Complete Surface Phase 9 — Crises & Safety matrix fill (carries the one correctness item in Movement II). **Fix the `inspection` actor asymmetry first**: faction-signal reads don't resolve when `primaryActor` is a `notable_npc`, silently dropping those seeds to single-condition rungs — a degradation, not a content choice. Then fill the `relationship = high` face (currently only `relationship = low` is authored) and the `foodSafety` / `violence` cubes (~8 of 27 cells each today), widening pickers or allowlisting per Phase 3. Prove both inspection actor paths resolve a salient line with a test. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-107/122 (crisis templates). | broken | open | 176 |
| ISSUE-145 | Complete Surface Phase 10 — Reputation, Rumour & Rivals matrix fill. Fill the selectively-authored gaps: the 5 low-traffic reputation axes (`cheap`/`filthy`/`strange`/`culinary_renown`/`goblinAuthentic`) with no matrix cell; rumour `target × target` and `memory × memory` pairs; rival `memory × memory`. Author-or-allowlist per Phase 3. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-123 (reputation/rumour/rival templates). | thin | open | 177 |
| ISSUE-146 | Complete Surface Phase 11 — Periodic & Narrative matrix fill. Fill the unfilled aggregate grids: monthly `cross-pressure × memory`; seasonal `cross-theme`. Author-or-allowlist per Phase 3. Nine-gate walk + harnesses green. Depends on ISSUE-138 + ISSUE-109/124 (periodic templates). | thin | open | 178 |
| ISSUE-147 | Complete Surface Phase 12 — The Coverage Gate (centrepiece). A standing `checkMatrixCoverage` sibling gate (multi-template, like `legibility` / `faithfulness`, NOT in `runAllGates`) that enumerates every migrated situation's matrix cells via the Phase-3 read and fails on any uncovered, non-allowlisted cell (named), plus a second rule folding in the Phase-1 harness/gate-exercise completeness check. Two failing fixtures (unallowlisted hole; allowlisted hole that passes). Asserts on cell coverage, not wording; fast enough for the default suite. Depends on ISSUE-138 (cell read + allowlist) + ISSUE-139…146 (≥3 filled clusters to be meaningful). | thin | open | 179 |
| ISSUE-148 | Complete Surface Phase 13 — Deepening, Pruning & Recalibration (**standing** — future iterations append, never strictly done; matches ISSUE-113 / ISSUE-130). Play it, reading for coverage-felt-in-play: deepen flat filled cells, watch widened pickers for noise, prune any cell proven dead, recalibrate `BAND_THRESHOLDS` / `SALIENCE_TABLES` where a cut-point or ordering reads wrong. Every change passes `runAllGates` + the coverage gate. Depends on ISSUE-147 (coverage gate keeps matrices whole). | thin | open | 180 |
