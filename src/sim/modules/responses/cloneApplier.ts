...
function applyStateChange(
  ...
  if (path.startsWith('areas.')) {
    ...
    if (!area) return { ...preview, applied: false, notes: [`unknown area ${id}`] }
    ...
    return { ...preview, applied: false, notes: [`unsupported area field ${field}`] }
  }

  if (path.startsWith('stock.')) {
    const [, id, field] = path.split('.') as [string, string, keyof StockState]
    const item = state.stock[id]
    if (!item) return { ...preview, applied: false, notes: [`unknown stock ${id}`] }
    if (field === 'quantity') {
    ...
    return { ...preview, applied: false, notes: [`unsupported stock field ${field}`] }
  }

  if (path.startsWith('staff.')) {
    const [, id, field] = path.split('.') as [string, string, keyof StaffState]
    const member = state.staff[id]
    if (!member) return { ...preview, applied: false, notes: [`unknown staff ${id}`] }
    if (
      field === 'morale' ||
      field === 'stress' ||
      field === 'fatigue' ||
  ...
  if (path.startsWith('customers.')) {
    ...
    if (!group) return { ...preview, applied: false, notes: [`unknown customer ${id}`] }
    if (
      field === 'satisfaction' ||
      field === 'patronage' ||
      field === 'loyalty' ||
  ...
  if (path.startsWith('reputation.')) {
    ...
    if (current === undefined) {
      return { ...preview, applied: false, notes: [`unknown reputation axis ${String(axis)}`] }
    }
...
function applyPressure(state: TavernState, preview: EffectPreview): EffectResult {
  const amount = preview.amount ?? 0
...
export function createCloneApplier(
  ...
  causesAdded: Array<{ source: string; target: string; readable: string; amount: number }>
  logs: ApplierLog[]
} {
